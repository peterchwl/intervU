...
def visualize(
    ...
    for person in list_persons:
        ...
        # print("x: " + str(keypoints[0][1].x) + " y: " + str(keypoints[0][1].y))
        nose = (keypoints[0][1].x, keypoints[0][1].y)
        right_eye = (keypoints[1][1].x, keypoints[1][1].y)
        left_eye = (keypoints[2][1].x, keypoints[2][1].y)
        right_shoulder = (keypoints[5][1].x, keypoints[5][1].y)
        left_shoulder = (keypoints[6][1].x, keypoints[6][1].y)


        # print("1: " + str(nose))
        # print("2: " + str(left_eye))
        # print("3: " + str(right_eye))
        # print("4: " + str(left_shoulder))
        # print("5: " + str(right_shoulder))

        # need dist from shoulders
        # need dist from ls to n
        # need dist from rs to n
        dist_shoulders = math.hypot(left_shoulder[0] - right_shoulder[0], left_shoulder[1] - right_shoulder[1])
        dist_left_nose = math.hypot(left_shoulder[0] - nose[0], left_shoulder[1] - nose[1])
        dist_right_nose = math.hypot(right_shoulder[0] - nose[0], right_shoulder[1] - nose[1])

        # print(dist_shoulders)
        # print(dist_left_nose)
        # print(dist_right_nose)

        arr = [dist_shoulders, dist_left_nose, dist_right_nose]
        ...
        for edge_pair, edge_color in KEYPOINT_EDGE_INDS_TO_COLOR.items():
            if (keypoints[edge_pair[0]].score > keypoint_threshold and
                ...
                # print("distance: " + str(math.dist(
                #     keypoints[edge_pair[0]].coordinate, keypoints[edge_pair[1]].coordinate)))
