...
def EyeTracking(image, gray, eyePoints):
    ...
    divPart = int(width/3)
    downDiv = int(width/4)
    divHeight = int(height/3)
    centerDiv = int(height/4)
    ...
    rightPart = thresholdEye[0:height, 0:downDiv]
    centerPart = thresholdEye[centerDiv:height - centerDiv, divPart:divPart+divPart]
    leftPart = thresholdEye[0:height, divPart+divPart:width]
    downPart = thresholdEye[centerDiv*2:height, downDiv: downDiv *2]
    upPart = thresholdEye[0:centerDiv*2, divPart:divPart+divPart]
    ...
    downBlackPx = np.sum(downPart == 0)
    upBlackpx = np.sum(upPart == 0)

    pos, color = Position([rightBlackPx, centerBlackPx, leftBlackPx, downBlackPx, upBlackpx])
    # print(pos)

    return mask, pos, color, rightBlackPx, centerBlackPx, leftBlackPx, downBlackPx, upBlackpx
